...
An [Agent Framework](https://github.com/microsoft/agent-framework) hosted agent that connects to a **remote MCP server** (GitHub) for tool discovery using the **Responses protocol**. Instead of defining tools locally, the agent discovers and invokes tools at runtime from an MCP-compatible endpoint — in this case, the GitHub Copilot MCP server. This enables dynamic tool integration without redeployment.
...
4. **GitHub Personal Access Token (PAT)**
   - Follow the instructions here to get a PAT: [Creating a personal access token](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/creating-a-personal-access-token)
...
| `GITHUB_PAT` | Yes | GitHub Personal Access Token for authenticating with the GitHub MCP server. |
...
> `azd ai agent init -m <path-to-repo>/samples/python/hosted-agents/agent-framework/responses/03-mcp/agent.manifest.yaml`
...
curl -sS -X POST http://localhost:8088/responses \
  ...
  -d '{"input": "List all the repositories I own on GitHub.", "stream": false}' | jq .
```
